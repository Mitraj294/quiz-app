<x-app-layout>
                        ...
                        <div class="p-4 bg-indigo-50 border border-indigo-200 rounded-lg" style="width: 500px;">
                            ...
                            <div class="mt-4 flex gap-2 justify-end">
                                <button type="button" onclick="applyDefaultsToAll()"
                                    class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150"> Apply to All
                                </button>
                                <button type="button" onclick="selectAllQuestions()"
                                    class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150"> Select All
                                </button>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function updateNegativeOptionsForQuestion(questionId) {
            ...
            if (found) negativeSelect.value = found.value;
            else negativeSelect.selectedIndex = 0;
        }
        ...
        function toggleDefaultNegativeMarks(value) {
            ...
            if (window.NegativeMarks) {
                ...
                if (value === 'yes') dropdown.classList.remove('hidden');
        ...
        function updateDefaultNegativeOptions() {
            ...
            if (window.NegativeMarks) {
                ...
                const options = [{
                    ...
                    {
                        value: +(marks / 4).toFixed(2),
                        label: `1/4 (${(marks / 4).toFixed(2)})`
                    },
                    {
                        value: +(marks / 3).toFixed(2),
                        label: `1/3 (${(marks / 3).toFixed(2)})`
                    },
                    {
                        value: +(marks / 2).toFixed(2),
                        label: `1/2 (${(marks / 2).toFixed(2)})`
                    },
                    {
                        value: +marks.toFixed(2),
                        label: `Full (${marks.toFixed(2)})`
                    },
                ];
                options.forEach(opt => {
