            ...
            <div class="max-w-full mx-auto py-6 px-4 sm:px-6 lg:px-8">
                ...
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    ...
                    @if($isAdmin && request()->has('user_id'))
                        {{ \App\Models\User::find(request()->input('user_id'))?->name ?? 'User' }}'s Analytics for {{ $quiz->name }}
                    @else
                        Results - {{ $quiz->name }}
                    @endif
                </h2>
    ...
    <div class="py-12">
        <div class="max-w-full mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-bold">
                        @if(Auth::user() && Auth::user()->isAdmin() && request()->has('user_id'))
                            {{ optional(\App\Models\User::find(request()->input('user_id')))->name ?? 'User' }}'s Attempts for {{ $quiz->name }}
                        @else
                ...
                <x-utc-converter />
                <div class="mt-6">
                    <a href="{{ url('/admin/analytics/users') }}" class="text-sm text-gray-600 hover:underline">Back to Users Analytics</a>
                </div>
            </div>
        </div>
    </div>
