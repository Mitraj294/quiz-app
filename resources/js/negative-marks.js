...
// Exposes functions on the global object so inline onchange handlers can call them.
...
function toggleNegativeSelectVisibility(toggleEl, selectEl) {
    let v;
    if (toggleEl && typeof toggleEl === 'object') {
        // Prefer .value (for inputs) then data-value, then attribute value as a fallback
        v = toggleEl.value ?? toggleEl.dataset?.value ?? (toggleEl.getAttribute ? toggleEl.getAttribute('value') : undefined);
    } else {
        v = toggleEl;
    }
...
function initNegativeMarksForPage() {
    ...
    if (defaultMarks) {
        ...
        updateNegativeOptionsForSelect(dd, defaultMarks.value, dd?.dataset?.selected);
    ...
    for (const input of document.querySelectorAll('.question-marks')) {
        const id = input.dataset.questionId;
        const marksVal = input.value ?? (input.getAttribute ? input.getAttribute('value') : undefined) ?? 1;
        ...
        if (negSelect) updateNegativeOptionsForSelect(negSelect, marksVal, negSelect.dataset?.selected);
...
export default globalThis.NegativeMarks;
